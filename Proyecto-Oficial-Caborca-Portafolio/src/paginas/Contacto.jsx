 ...
 * Consume textos dinámicos del CMS y envía formularios al backend.
 */
const Contacto = () => {
  ...
  // Carga datos globales de contacto y redes desde ConfiguracionGeneral.
  useEffect(() => {
  ...
  // Carga textos editables específicos de la página Contacto.
  useEffect(() => {
    textosService.getTextos('contacto')
      .then(data => {
        ...
        if (data.seccionAyuda) setSeccionAyuda(prev => ({ ...prev, ...data.seccionAyuda }));
        if (data.seccionMapa) setSeccionMapa(prev => ({ ...prev, ...data.seccionMapa }));
      })
      .catch(() => console.warn('Contacto: usando datos por defecto'));
  }, []);

  const labels = {
    ayuda: language === 'es' ? seccionAyuda.titulo_ES : seccionAyuda.titulo_EN,
    equipo: language === 'es' ? seccionAyuda.subtitulo_ES : seccionAyuda.subtitulo_EN,
  ...
  // Envía formulario de contacto y muestra feedback al usuario.
      ...
      <main>
        ...
        <section className="relative bg-gray-40">
          <div className="relative w-full overflow-hidden shadow-2xl">
            <img src={getImageUrl(hero.imagen)} alt="Contacto Caborca Boots" className="w-full h-screen object-cover"
