...
/**
 * Vista de detalle de producto para ambos catálogos.
 * Resuelve producto por ruta y evita fugas de idioma en campos localizados.
 */
const DetalleProducto = () => {
  ...
  useEffect(() => {
    // Carga ambos catálogos y busca el producto por id y catálogo padre.
    setLoading(true);
    Promise.all([
      ...
      if (found) {
        ...
        if (found.imagenes && found.imagenes.length > 0) {
          setImagenPrincipal(getImageUrl(found.imagenes[0]));
        } else if (found.imagen) {
          setImagenPrincipal(getImageUrl(found.imagen));
        }
  ...
  // Determina si un valor puede mostrarse en UI.
  ...
  // Lee campo por idioma activo y usa fallback neutral solo cuando corresponde.
  const getProductText = (item, field, options = {}) => {
    ...
    const hasLocalizedKeys = Object.prototype.hasOwnProperty.call(item, keyES)
      || Object.prototype.hasOwnProperty.call(item, keyEN);

    if (!hasLocalizedKeys && allowNeutral && hasValue(item[field])) return item[field];
  ...
  const getSkuDisplay = (item) => {
    const localized = getProductText(item, 'sku', { allowNeutral: false, fallback: '' });
    if (hasValue(localized)) return localized;
    const neutral = item?.sku;
    if (hasValue(neutral) && isSkuCode(String(neutral).trim())) return String(neutral).trim();
    return '';
  };

  const getCategoryLabel = (item) => {
    const localized = getProductText(item, 'categoria', { allowNeutral: false, fallback: '' });
    if (hasValue(localized)) return localized;

    const neutral = String(item?.categoria || '').trim();
    if (!neutral) return '';
  ...
  // Arma breadcrumb dinámico según catálogo y nombre localizado.
  const breadcrumbs = useMemo(() => {
    ...
    return [
      ...
      { name: getProductText(producto, 'nombre'), active: true }
    ];
  }, [producto, language]);
  ...
  const imagenes = (producto.imagenes && producto.imagenes.length > 0 ? producto.imagenes : [producto.imagen]).map(getImageUrl);
  const materialesRaw = getProductText(producto, 'materiales');
  const materialesTexto = Array.isArray(materialesRaw) ? materialesRaw.join(', ') : materialesRaw;
            ...
            <nav className="text-sm text-caborca-cafe flex items-center gap-2">
              {breadcrumbs.map((bc, idx) => (
                <div key={idx} className="flex items-center gap-2">
                  {bc.path ? (
                    <Link to={bc.path} className="hover:underline text-caborca-cafe font-semibold text-xs sm:text-sm">{bc.name}</Link>
                  ) : (
                    <span className="text-caborca-cafe font-bold text-xs sm:text-sm truncate max-w-37.5">{bc.name}</span>
                  )}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16">
              ...
              <div className="h-fit">
                ...
                >
                  <img
                    src={imagenPrincipal}
                    alt={getProductText(producto, 'nombre')}
                ...
                <div className="grid grid-cols-4 gap-2 sm:gap-3">
                  {imagenes.map((img, idx) => (
                    <div
                      key={idx}
                      onClick={() => {
                        setImagenPrincipal(getImageUrl(img));
              ...
              <div>
                ...
                <h1 className="text-3xl sm:text-4xl md:text-5xl font-serif text-caborca-beige-fuerte font-bold mb-2">
                  {getProductText(producto, 'nombre')}
                </h1>

                <p className="text-sm text-gray-500 mb-4 font-medium uppercase">
                  {getSkuDisplay(producto) ? `SKU: ${getSkuDisplay(producto)}` : `CAT: ${getCategoryLabel(producto)}`}
                </p>

                <div className="text-gray-600 mb-8 leading-relaxed space-y-4 text-base sm:text-lg">
                  <p>
                    {getProductText(producto, 'descripcion')}
                  </p>
                </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8 text-sm">
                    {materialesTexto && (
                      <div className="bg-gray-50 p-4 rounded-lg border border-gray-100">
                        ...
                        <span className="font-bold text-caborca-cafe text-base capitalize">{materialesTexto}</span>
                      </div>
                    )}
                    {getProductText(producto, 'materialCorte') && (
                      <div className="bg-gray-50 p-4 rounded-lg border border-gray-100">
                        ...
                        <span className="font-bold text-caborca-cafe text-base">{getProductText(producto, 'materialCorte')}</span>
                      </div>
                    )}
                    {getProductText(producto, 'suela') && (
                      <div className="bg-gray-50 p-4 rounded-lg border border-gray-100">
                        <span className="block text-gray-400 text-[10px] uppercase tracking-wider font-bold mb-1">{language === 'es' ? 'Suela' : 'Sole'}</span>
                        <span className="font-bold text-caborca-cafe text-base">{getProductText(producto, 'suela')}</span>
                      </div>
                    )}
                    {getProductText(producto, 'construccion') && (
                      <div className="bg-gray-50 p-4 rounded-lg border border-gray-100">
                        ...
                        <span className="font-bold text-caborca-cafe text-base">{getProductText(producto, 'construccion')}</span>
                      </div>
                    )}
                    {getProductText(producto, 'horma') && (
                      <div className="bg-gray-50 p-4 rounded-lg border border-gray-100">
                        <span className="block text-gray-400 text-[10px] uppercase tracking-wider font-bold mb-1">{language === 'es' ? 'Horma' : 'Last'}</span>
                        <span className="font-bold text-caborca-cafe text-base">{getProductText(producto, 'horma')}</span>
                      </div>
                    )}
                  </div>
