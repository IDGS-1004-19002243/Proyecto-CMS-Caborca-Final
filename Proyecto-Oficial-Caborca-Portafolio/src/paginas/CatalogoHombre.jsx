...
/**
 * Página pública de catálogo de hombre.
 * Incluye filtros y render estricto por idioma para evitar mezcla ES/EN.
 */
const CatalogoHombre = () => {
  ...
  useEffect(() => {
    homeService.getCatalogoHombre().then(data => {
      ...
      if (data && data.contenido) {
        setContenido(prev => ({ ...prev, ...data.contenido }));
      }
    }).catch(console.error);
  }, []);

  // Verifica si el campo tiene valor utilizable para render.
  ...
  // Selecciona texto por idioma con fallback neutral controlado.
  const getProductText = (producto, field, options = {}) => {
  ...
  const getSkuDisplay = (producto) => {
    const localized = getProductText(producto, 'sku', { allowNeutral: false, fallback: '' });
    if (hasValue(localized)) return localized;
    const neutral = producto?.sku;
    if (hasValue(neutral) && isSkuCode(String(neutral).trim())) return String(neutral).trim();
    return '';
  };

  const getCategoryLabel = (producto) => {
    const localized = getProductText(producto, 'categoria', { allowNeutral: false, fallback: '' });
    if (hasValue(localized)) return localized;
  ...
  // Aplica filtros y ordenamiento manteniendo textos localizados.
  const productosFiltrados = useMemo(() => {
    ...
    if (ordenarPor === 'nombre') {
      lista.sort((a, b) => (getProductText(a, 'nombre') || '').localeCompare(getProductText(b, 'nombre') || ''));
    } else if (ordenarPor === 'nuevo') {
      lista.sort((a, b) => (b.id || 0) - (a.id || 0));
    }
    return lista;
  }, [listaProductos, estilo, ordenarPor, language]);
        ...
        <section className="pt-24 sm:pt-32 pb-10 sm:pb-14" style={{ backgroundColor: '#F0EBE3' }}>
          <div className="container mx-auto px-4 text-center">
            <h1 className="text-4xl sm:text-5xl md:text-6xl font-serif mb-4 sm:mb-6 text-caborca-cafe">
              {language === 'en' ? (contenido?.titulo_EN || labels.titulo) : (contenido?.titulo_ES || contenido?.titulo || labels.titulo)}
            </h1>
            ...
            <p className="text-caborca-cafe text-base sm:text-lg md:text-xl max-w-3xl mx-auto leading-relaxed">
              {language === 'en' ? (contenido?.subtitulo_EN || labels.subtitulo) : (contenido?.subtitulo_ES || contenido?.subtitulo || labels.subtitulo)}
            </p>
          </div>
        </section>
                  ...
                  productosFiltrados.map((producto) => (
                    <div key={producto.id} className="group relative">
                      <div className="relative overflow-hidden rounded-lg shadow-lg aspect-3/4 bg-gray-100">
                        <img
                          src={getImageUrl((producto.imagenes && producto.imagenes[0]) || producto.imagen)}
                      ...
                      <div className="mt-6 text-center">
                        <h3 className="text-xl font-serif font-bold text-caborca-cafe mb-1">
                          <Link to={`/producto/hombre/${producto.id}`}>{getProductText(producto, 'nombre')}</Link>
                        </h3>
                        <p className="text-sm text-gray-500 mb-2 font-medium uppercase tracking-wide">
                          {getSkuDisplay(producto) ? `SKU: ${getSkuDisplay(producto)}` : getCategoryLabel(producto)}
                        </p>
                      </div>
                    </div>
                  ))
