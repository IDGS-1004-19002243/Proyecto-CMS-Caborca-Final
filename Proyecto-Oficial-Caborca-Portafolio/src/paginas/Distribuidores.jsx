...
const formatAddress = (store, language = 'es') => {
  const addressEs = [store.calle, store.colonia, store.ciudad, store.estado, store.cp].filter(Boolean).join(', ');
  const addressEn = store.direccion_EN || store.address_EN || [
    store.calle,
    store.colonia_EN || store.colonia,
    store.ciudad_EN || store.ciudad,
    store.estado_EN || store.estado,
    store.cp,
  ].filter(Boolean).join(', ');
  return language === 'en' ? (addressEn || addressEs) : (addressEs || addressEn);
};

const getStoreCity = (store, language = 'es') => {
  return language === 'en' ? (store.ciudad_EN || store.ciudad || '') : (store.ciudad || store.ciudad_EN || '');
};

const getStoreState = (store, language = 'es') => {
  return language === 'en' ? (store.estado_EN || store.estado || '') : (store.estado || store.estado_EN || '');
};

const normalizeTipoVenta = (tipo) => {
  const value = (tipo || '').toLowerCase().trim();
  if (value === 'ambos') return 'ambas';
  if (value === 'tienda' || value === 'online' || value === 'ambas') return value;
  return '';
};

/**
 * Página pública de distribuidores.
 * Integra listado filtrable, mapa Leaflet y solicitud de nuevos distribuidores.
 */
const Distribuidores = () => {
  ...
  // Carga textos CMS para hero/formulario/mapa de la página.
  useEffect(() => {
    textosService.getTextos('distribuidores')
      .then(data => {
        ...
        if (data.hero) {
          setHero(prev => ({ ...prev, ...cleanEmptyStrings(data.hero) }));
        }
        if (data.formulario || data.counters) {
          setFormDist(prev => ({
            ...prev,
            ...cleanEmptyStrings(data.formulario),
            distribuidores_ES: data.counters?.distribuidores_ES || data.counters?.distribuidores || prev.distribuidores_ES,
            distribuidores_EN: data.counters?.distribuidores_EN || prev.distribuidores_EN || data.counters?.distribuidores_ES || data.counters?.distribuidores || prev.distribuidores_ES,
            estados_ES: data.counters?.estados_ES || data.counters?.estados || prev.estados_ES,
            estados_EN: data.counters?.estados_EN || prev.estados_EN || data.counters?.estados_ES || data.counters?.estados || prev.estados_ES,
          }));
        }
        setMapInfo(prev => ({
          ...prev,
          mapTitle_ES: data.mapTitle_ES || data.mapTitle || prev.mapTitle_ES,
          mapTitle_EN: data.mapTitle_EN || prev.mapTitle_EN,
          mapText_ES: data.mapText_ES || data.mapText || prev.mapText_ES,
          mapText_EN: data.mapText_EN || prev.mapText_EN,
        }));
      })
      .catch(() => { });
  }, []);

  // Carga listado de distribuidores desde configuración general.
  useEffect(() => {
        ...
        if (data && Array.isArray(data.distribuidoresList) && data.distribuidoresList.length > 0) {
          const mapped = data.distribuidoresList
            ...
            .map(d => ({
              ...
              ciudad: d.ciudad || '',
              ciudad_EN: d.ciudad_EN || d.ciudadEn || d.city_EN || '',
              estado: d.estado || '',
              estado_EN: d.estado_EN || d.estadoEn || d.state_EN || '',
              pais: d.pais || '',
              colonia: d.colonia || '',
              colonia_EN: d.colonia_EN || d.coloniaEn || '',
              calle: d.calle || '',
              numeroExt: d.numeroExt || '',
              cp: d.cp || '',
              direccion_EN: d.direccion_EN || d.direccionEn || d.address_EN || '',
              tipo: normalizeTipoVenta(d.tipoVenta || d.tipo) || 'tienda',
              lat: parseFloat(d.lat),
              lng: parseFloat(d.lng),
              logo: (d.imagenes && d.imagenes.length > 0) ? d.imagenes[0] : (d.logo || null),
  ...
  const regionesList = useMemo(() => {
    ...
    return allDistribuidores
      .map((d) => getStoreState(d, language))
      .filter((estado) => estado && !seen.has(estado) && seen.add(estado))
      .sort((a, b) => a.localeCompare(b, 'es'));
  }, [allDistribuidores, language]);
  ...
  const labels = {
    ...
    tipoVentaLabel: language === 'es' ? 'Tipo de venta' : 'Sales type',
    direccionLabel: language === 'es' ? 'Dirección' : 'Address',
    telefonoLabel: language === 'es' ? 'Teléfono' : 'Phone',
    websiteLabel: language === 'es' ? 'Sitio web' : 'Website',
    noDisponible: language === 'es' ? 'No disponible' : 'Not available',
  };
  ...
  const manejarAplicarFiltros = () => {
    ...
    if (estadoFiltro) filtrados = filtrados.filter(d => getStoreState(d, language) === estadoFiltro);
      ...
      <main>
        ...
        <section className="relative bg-gray-50">
          <div className="relative w-full overflow-hidden shadow-2xl">
            <img src={getImageUrl(hero.imagen)} alt="Caborca" className="w-full h-screen object-cover" />
                ...
                <div className="flex flex-col md:flex-row items-center justify-between gap-8 pt-6 border-t border-gray-100">
                  ...
                  <div className="flex items-center gap-10">
                    <div className="text-center">
                      <div className="text-3xl font-bold text-caborca-cafe">{t(formDist, 'distribuidores')}</div>
                      <div className="text-xs text-gray-400 uppercase font-bold">{labels.distribuidoresLabel}</div>
                    </div>
                    <div className="text-center">
                      <div className="text-3xl font-bold text-caborca-cafe">{t(formDist, 'estados')}</div>
        ...
        <section className="py-20 bg-gray-50">
          <div className="container px-4 max-w-7xl mx-auto">
                ...
                {resultados.map((store, i) => (
                  <div key={i} onClick={() => seleccionarTienda(store)} className={`p-4 rounded-xl border-2 cursor-pointer transition-all ${selectedStore === store ? 'border-caborca-cafe bg-white shadow-lg' : 'border-transparent bg-white/50 hover:border-gray-300'}`}>
                    <div className="flex gap-4 items-center mb-2">
                      {store.logo && <img src={getImageUrl(store.logo)} alt="logo" className="w-10 h-10 object-contain rounded bg-white p-1" />}
                      <p className="font-bold text-caborca-cafe truncate">{language === 'en' ? (store.nombre_EN || store.nombre || 'Distributor') : (store.nombre || store.nombre_EN || 'Distribuidor')}</p>
                    </div>
                      <p className="text-sm text-gray-500 mb-2 truncate">{getStoreCity(store, language)}, {getStoreState(store, language)}</p>
            ...
            <div className="h-150 rounded-3xl overflow-hidden shadow-2xl relative z-10">
                      ...
                      <div className="w-110 max-w-[78vw] rounded-xl border border-caborca-bronce/40 bg-white p-2.5 shadow-lg text-caborca-cafe">
                        <div className="flex items-center gap-3">
                          <div className="h-20 w-20 shrink-0 rounded-md bg-caborca-arena/35 p-1 flex items-center justify-center">
                            {store.logo ? (
                              <img src={getImageUrl(store.logo)} alt={language === 'en' ? (store.nombre_EN || store.nombre || 'Distributor') : (store.nombre || store.nombre_EN || 'Distribuidor')} className="h-full w-full object-contain rounded-md" />
                            ) : (
                          ...
                          <div className="min-w-0 flex-1 flex flex-col justify-center">
                            <div className="flex items-start justify-between gap-2">
                              <p className="m-0 font-bold text-xl leading-none wrap-break-word">{language === 'en' ? (store.nombre_EN || store.nombre || 'Distributor') : (store.nombre || store.nombre_EN || 'Distribuidor')}</p>
                              <span className="shrink-0 rounded-full bg-caborca-arena px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wide text-caborca-cafe">
                                {store.tipo === 'tienda' ? labels.tiendaFisica : store.tipo === 'online' ? labels.enLinea : labels.ambos}
                              </span>
                            </div>

                            <div className="text-[11px] leading-snug">
                              <p><span className="font-semibold">{labels.direccionLabel}:</span> {formatAddress(store, language) || labels.noDisponible}</p>
                              <p>
                                <span className="font-semibold">{labels.telefonoLabel}:</span>{' '}
                                {store.telefono ? (
                                  <a href={`tel:${store.telefono}`} className="font-semibold text-caborca-bronce underline">{store.telefono}</a>
                                ) : labels.noDisponible}
                              </p>
                              <p>
                                <span className="font-semibold">{labels.websiteLabel}:</span>{' '}
                                {store.sitioWeb ? (
                                  <a href={normalizeWebsiteUrl(store.sitioWeb)} target="_blank" rel="noreferrer" className="font-semibold text-caborca-bronce break-all underline">{store.sitioWeb}</a>
                                ) : labels.noDisponible}
                              </p>
                            </div>
                          </div>
            ...
            {allDistribuidores.length === 0 && (
              <p className="text-center text-sm text-caborca-cafe mt-4">
                {language === 'es' ? 'No hay distribuidores registrados con coordenadas para mostrar en el mapa.' : 'There are no registered distributors with coordinates to show on the map.'}
              </p>
            )}
          </div>
        </section>
