...
const defaultContent = {
  ...
  caborcaHoy: { title_ES: 'Caborca Group Hoy', title_EN: 'Caborca Group Today', subtitle_ES: 'Números que reflejan nuestro compromiso y crecimiento', subtitle_EN: 'Numbers that reflect our commitment and growth', stats: [{ label_ES: 'AÑOS DE HISTORIA', label_EN: 'YEARS OF HISTORY', value_ES: '45', value_EN: '45' }, { label_ES: 'MARCAS PROPIAS', label_EN: 'OWN BRANDS', value_ES: '5', value_EN: '5' }, { label_ES: 'COLABORADORES', label_EN: 'WORKERS', value_ES: '800+', value_EN: '800+' }, { label_ES: 'PARES SEMANALES', label_EN: 'WEEKLY PAIRS', value_ES: '8,000', value_EN: '8,000' }], paragraph_ES: 'En la actualidad Caborca Group cuenta con <strong>4 plantas manufactureras</strong> cubriendo un área total de <strong>30,000 metros cuadrados</strong>.', paragraph_EN: 'Today Caborca Group has <strong>4 manufacturing plants</strong> covering a total area of <strong>30,000 square meters</strong>.' },
  ...
  proceso: { badge_ES: 'NUESTRO PROCESO', badge_EN: 'OUR PROCESS', title_ES: 'Pasión por el detalle', title_EN: 'Passion for Detail', paragraphs_ES: ['Todo lo que hacemos en Grupo Caborca lo hacemos con pasión.', 'Nuestras botas pasan por un proceso de más de 200 pasos.'], paragraphs_EN: ['Everything we do at Caborca Group we do with passion.', 'Our boots go through a process of more than 200 steps.'], stat_ES: '+200', stat_EN: '+200', statLabel_ES: 'Pasos en el proceso', statLabel_EN: 'Steps in the process', imagen: 'https://blocks.astratic.com/img/general-img-landscape.png' },
 ...
 * Renderiza historia corporativa con bloques dinámicos editables desde CMS.
 */
const Nosotros = () => {
  ...
  // Carga contenido desde CMS y conserva fallback de datos por defecto.
  useEffect(() => {
  ...
  // Mapea contenido crudo a estructura final de render según idioma activo.
  const content = useMemo(() => {
    ...
    const getProcesoStat = (obj) => {
      if (!obj || typeof obj !== 'object') return '';

      if (obj.stat_ES || obj.stat_EN) {
        return t(obj, 'stat');
      }
      ...
      if (legacyLooksText) {
        return t(obj, 'statLabel') || legacyValue;
      }
    ...
    const getStatValue = (stat) => {
      ...
      // Camino nuevo: valor bilingue
      if (stat.value_ES || stat.value_EN) {
        return t(stat, 'value');
      }

      // Camino legacy: si `value` es texto libre, usar label por idioma para respetar selector ES/EN
      const legacyValue = stat.value ?? '';
      const legacyLooksText = typeof legacyValue === 'string' && /[A-Za-zÁÉÍÓÚáéíóúÑñ]/.test(legacyValue);
      if (legacyLooksText) {
        return t(stat, 'label') || legacyValue;
      }

      // Valores numericos legacy se mantienen
      return legacyValue;
    };
    ...
    return {
      ...
      proceso: {
        ...
        stat: getProcesoStat(rawContent.proceso),
      ...
      <main>
        ...
        <section className="relative bg-gray-50">
          <div className="relative w-full overflow-hidden shadow-2xl">
            <img src={getImageUrl(content.hero?.imagen)} alt="Nosotros Caborca" className="w-full h-screen object-cover" />
          ...
          <div className="container mx-auto px-4">
            <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 items-center">
              ...
              <div className="relative">
                <img src={getImageUrl(content.origen?.imagen)} alt="Origen" className="rounded-2xl shadow-2xl w-full aspect-3/2 object-cover" />
              </div>
            </div>
          </div>
        </section>
          ...
          <div className="container mx-auto px-4">
            <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 items-center">
              <div className="order-2 lg:order-1">
                <img src={getImageUrl(content.crecimiento?.imagen)} alt="Crecimiento" className="rounded-2xl shadow-2xl w-full aspect-3/2 object-cover" />
              </div>
        ...
        <section className="py-20 bg-white">
          <div className="container mx-auto px-4 max-w-6xl grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 items-center">
            ...
            <div>
              <img src={getImageUrl(content.artesania?.imagen)} alt="Artesanía" className="rounded-2xl shadow-xl w-full aspect-3/2 object-cover" />
            </div>
          </div>
        </section>
        ...
        <section className="py-20 bg-gray-50">
          <div className="container mx-auto px-4 max-w-6xl grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 items-center">
            <div className="order-2 lg:order-1">
              <img src={getImageUrl(content.proceso?.imagen)} alt="Proceso" className="rounded-2xl shadow-xl w-full aspect-3/2 object-cover" />
            </div>
            <div className="order-1 lg:order-2">
              <div className="inline-block bg-caborca-beige-suave px-6 py-2 rounded-full mb-6 uppercase font-bold text-sm text-caborca-cafe">
