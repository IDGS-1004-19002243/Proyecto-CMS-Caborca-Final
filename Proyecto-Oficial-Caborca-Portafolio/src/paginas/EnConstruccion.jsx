...
/**
 * Vista pública de mantenimiento.
 * Muestra mensaje temporal y accesos sociales configurados en CMS.
 */
const EnConstruccion = () => {
    ...
    useEffect(() => {
        // Carga configuración de mantenimiento y migra formato legacy de redes.
    ...
    const socialIcons = {
        ...
        email: <svg className="w-6 h-6" fill="currentColor" viewBox="0 2 24 24"><path d="M22 6c0-1.1-.9-2-2-2H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6zm-2 0l-8 5-8-5h16zm0 12H4V8l8 5 8-5v10z" /></svg>
    };
    ...
    return (
        <div className="relative min-h-screen flex items-center justify-center bg-gray-900 text-white font-sans overflow-hidden">
            <div className="absolute inset-0 z-0">
                <img src={getImageUrl(content.imagenFondo)} alt="Maintenance" className="w-full h-full object-cover opacity-50" />
            ...
            <div className="container mx-auto px-4 text-center relative z-10 max-w-4xl flex flex-col items-center justify-center">
                ...
                <div className="flex justify-center gap-6 mt-4">
                    {['whatsapp', 'facebook', 'instagram', 'tiktok', 'email'].map(key => {
                        ...
                        const href = key === 'email' ? (config.url.startsWith('mailto:') ? config.url : `mailto:${config.url}`) : config.url;
                        
                        return (
                            <a key={key} href={href} target={key === 'email' ? '_self' : '_blank'} rel="noopener noreferrer"
                                className="text-white opacity-80 transition-all duration-300 hover:scale-110 drop-shadow-md"
