...
/**
 * Header público del portafolio.
 * Gestiona navegación desktop/mobile, selector de idioma y comportamiento del logo.
 */
const Header = () => {
  // Estado del menú móvil y submenú de catálogo.
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false)
  const [mobileDropdownOpen, setMobileDropdownOpen] = useState(false)
  // Controla estilo translúcido/solido al hacer hover sobre barra superior.
  const [hovered, setHovered] = useState(false)
  const { language, setLanguage } = useLanguage()

  /** Abre/cierra el menú lateral móvil. */
  const toggleMobileMenu = () => {
    setMobileMenuOpen(!mobileMenuOpen)
  }

  /**
   * Cambia idioma global de la sesión desde la UI.
  ...
  /**
   * Fuerza recarga completa al hacer click en logo.
   * - Si está en `/`, hace reload.
   * - Si está en otra ruta, navega al home con carga completa.
   */
    ...
    <>
      ...
      >
        <div className="container mx-auto px-4 sm:px-6 py-6 flex justify-between items-center">
          ...
          {/* Desktop Navigation */}
          <nav className="hidden lg:flex grow justify-center items-center gap-4 xl:gap-8 text-xs xl:text-sm font-medium tracking-wider">
            {/* Dropdown Botas */}
