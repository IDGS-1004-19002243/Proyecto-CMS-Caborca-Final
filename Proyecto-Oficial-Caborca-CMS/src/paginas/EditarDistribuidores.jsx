...
/**
 * Editor CMS de la página de distribuidores.
 * Controla hero, formulario, filtros y bloque de mapa en dos idiomas.
 */
export default function EditarDistribuidores() {
  const { success, error: toastError, info } = useToast();
  const { lang: idioma = 'es' } = useOutletContext();

  // Contenido base usado como fallback si la API regresa campos incompletos.
  const defaultContent = {
    hero: {
      badge_ES: 'ÚNETE A NOSOTROS', badge_EN: 'JOIN US',
      titulo_ES: '¿Quieres ser distribuidor?', titulo_EN: 'Do you want to be a distributor?',
      subtitulo_ES: 'Únete a nuestra red de distribuidores y forma parte de la familia Caborca', subtitulo_EN: 'Join our network of distributors and become part of the Caborca family',
      imagen: 'https://blocks.astratic.com/img/general-img-landscape.png'
    },
    counters: {
      distribuidores_ES: '+500',
      distribuidores_EN: '+500',
      estados_ES: '20+',
      estados_EN: '20+'
    },
    formulario: {
      titulo_ES: '¿Quieres ser distribuidor?', titulo_EN: 'Do you want to be a distributor?',
      subtitulo_ES: 'Si estás interesado, déjanos tus datos y nuestro equipo se pondrá en contacto contigo.', subtitulo_EN: 'If you are interested, leave us your details and our team will get in touch with you.',
  ...
  const [form, setForm] = useState({
    ...
    distribuidores_ES: '', distribuidores_EN: '', estados_ES: '', estados_EN: '',
  ...
  // Estado del formulario público de "quiero ser distribuidor" en la vista previa.
  ...
  // Inicializa formulario del modal según la sección seleccionada.
  function openEditor(section) {
    ...
    if (section === 'hero') {
      setForm({
        ...form,
        badge_ES: data.badge_ES || data.badge || dft.badge_ES || '',
        badge_EN: data.badge_EN || dft.badge_EN || '',
        titulo_ES: data.titulo_ES || data.titulo || dft.titulo_ES || '',
        titulo_EN: data.titulo_EN || dft.titulo_EN || '',
        subtitulo_ES: data.subtitulo_ES || data.subtitulo || dft.subtitulo_ES || '',
        subtitulo_EN: data.subtitulo_EN || dft.subtitulo_EN || '',
        imagen: data.imagen || dft.imagen || null,
      });
    } else if (section === 'formulario') {
      setForm({
        ...form,
        badge_ES: data.badge_ES || '',
        badge_EN: data.badge_EN || '',
        titulo_ES: data.titulo_ES || data.titulo || dft.titulo_ES || '',
        titulo_EN: data.titulo_EN || dft.titulo_EN || '',
        subtitulo_ES: data.subtitulo_ES || data.subtitulo || dft.subtitulo_ES || '',
        subtitulo_EN: data.subtitulo_EN || dft.subtitulo_EN || '',
        submitLabel_ES: data.submitLabel_ES || data.submitLabel || dft.submitLabel_ES || '',
        submitLabel_EN: data.submitLabel_EN || dft.submitLabel_EN || '',
        responseMessage_ES: data.responseMessage_ES || data.responseMessage || dft.responseMessage_ES || '',
        responseMessage_EN: data.responseMessage_EN || dft.responseMessage_EN || '',
        responseTime_ES: data.responseTime_ES || data.responseTime || dft.responseTime_ES || '',
        responseTime_EN: data.responseTime_EN || dft.responseTime_EN || '',
        distribuidores_ES: content.counters?.distribuidores_ES || content.counters?.distribuidores || dft.distribuidores_ES || dft.distribuidores || '',
        distribuidores_EN: content.counters?.distribuidores_EN || dft.distribuidores_EN || content.counters?.distribuidores_ES || content.counters?.distribuidores || dft.distribuidores_ES || dft.distribuidores || '',
        estados_ES: content.counters?.estados_ES || content.counters?.estados || dft.estados_ES || dft.estados || '',
        estados_EN: content.counters?.estados_EN || dft.estados_EN || content.counters?.estados_ES || content.counters?.estados || dft.estados_ES || dft.estados || ''
      });
    } else if (section === 'mapa') {
      setForm({ 
        ...form, 
        mapSrc: content.mapSrc || dft.mapSrc || '', 
        mapTitle_ES: content.mapTitle_ES || content.mapTitle || dft.mapTitle_ES || '', 
        mapTitle_EN: content.mapTitle_EN || dft.mapTitle_EN || '', 
        mapText_ES: content.mapText_ES || content.mapText || dft.mapText_ES || '', 
        mapText_EN: content.mapText_EN || dft.mapText_EN || '' 
      });
    } else if (section === 'filtros') {
      setForm({ 
        ...form, 
        purchasePlaceholder_ES: data.purchasePlaceholder_ES || data.purchasePlaceholder || dft.purchasePlaceholder_ES || '', 
        purchasePlaceholder_EN: data.purchasePlaceholder_EN || dft.purchasePlaceholder_EN || '', 
        estadoPlaceholder_ES: data.estadoPlaceholder_ES || data.estadoPlaceholder || dft.estadoPlaceholder_ES || '', 
        estadoPlaceholder_EN: data.estadoPlaceholder_EN || dft.estadoPlaceholder_EN || '' 
      });
    }
  ...
  // Aplica cambios del modal a la sección activa y los persiste en backend.
  const saveChanges = async () => {
    ...
    try {
      ...
      if (activeEdit === 'hero') {
        ...
        const c = {
          ...newContent.counters,
          distribuidores_ES: form.distribuidores_ES !== undefined ? form.distribuidores_ES : (newContent.counters?.distribuidores_ES || newContent.counters?.distribuidores || ''),
          distribuidores_EN: form.distribuidores_EN !== undefined ? form.distribuidores_EN : (newContent.counters?.distribuidores_EN || newContent.counters?.distribuidores_ES || newContent.counters?.distribuidores || ''),
          estados_ES: form.estados_ES !== undefined ? form.estados_ES : (newContent.counters?.estados_ES || newContent.counters?.estados || ''),
          estados_EN: form.estados_EN !== undefined ? form.estados_EN : (newContent.counters?.estados_EN || newContent.counters?.estados_ES || newContent.counters?.estados || '')
        };
  ...
  const currentCounter = (obj, field) => {
      ...
      <main>
              ...
              <div className="relative overflow-hidden shadow-sm">
                ...
                <div className="absolute inset-0 bg-black/35 flex items-center justify-center">
                  <div className="text-center text-white px-6 flex flex-col items-center justify-center h-full">
                    ...
                    <h1 className="font-serif font-bold text-4xl md:text-5xl lg:text-6xl leading-tight mb-4 max-w-3xl mx-auto text-white">
                    ...
                    <div className="border-t border-gray-200 mt-6 pt-4">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <label className="block">
                          ...
                          <input name={idioma === 'es' ? 'distribuidores_ES' : 'distribuidores_EN'} value={idioma === 'es' ? (form.distribuidores_ES || '') : (form.distribuidores_EN || form.distribuidores_ES || '')} onChange={handleInput} className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:border-caborca-cafe focus:outline-none" />
                        </label>
                        <label className="block">
                          <div className="text-xs font-semibold text-gray-500 mb-1">Estados (Ej: 20+)</div>
                          <input name={idioma === 'es' ? 'estados_ES' : 'estados_EN'} value={idioma === 'es' ? (form.estados_ES || '') : (form.estados_EN || form.estados_ES || '')} onChange={handleInput} className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:border-caborca-cafe focus:outline-none" />
                        </label>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col md:flex-row items-center justify-between gap-6 pt-4">
                    ...
                    <div className="flex items-center gap-8">
                      <div className="text-center">
                        <div className="text-2xl font-bold text-caborca-cafe">{currentCounter(content.counters, 'distribuidores')}</div>
                      ...
                      <div className="text-center">
                        <div className="text-2xl font-bold text-caborca-cafe">{currentCounter(content.counters, 'estados')}</div>
        ...
        <section data-cms-section="mapa" className="py-8 bg-gray-100">
          <div className="container mx-auto px-4 relative">
            <EditButton section="mapa" onOpen={() => openEditor('mapa')} className="absolute top-0 right-4 z-10" />
