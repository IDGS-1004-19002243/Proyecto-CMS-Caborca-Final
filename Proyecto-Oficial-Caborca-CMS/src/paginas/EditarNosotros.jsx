...
/**
 * Editor CMS de la página "Nosotros".
 * Coordina múltiples bloques narrativos, estadísticas e imágenes bilingües.
 */
const EditarNosotros = () => {
    const context = useOutletContext();
    const idioma = context?.lang || 'es';

    // Fallback estructural para render y edición cuando faltan datos remotos.
    const defaultContent = {
        ...
        caborcaHoy: {
            ...
            stats: [
                { label_ES: 'AÑOS DE HISTORIA', label_EN: 'YEARS OF HISTORY', value_ES: '45', value_EN: '45' },
                { label_ES: 'MARCAS PROPIAS', label_EN: 'OWN BRANDS', value_ES: '5', value_EN: '5' },
                { label_ES: 'COLABORADORES', label_EN: 'WORKERS', value_ES: '800+', value_EN: '800+' },
                { label_ES: 'PARES SEMANALES', label_EN: 'WEEKLY PAIRS', value_ES: '8,000', value_EN: '8,000' }
            ],
    ...
    const [form, setForm] = useState({
        ...
        stat_ES: '', stat_EN: '', image: null, stats: []
    });

    const { success, error } = useToast();

    useEffect(() => {
        // Hidrata contenido desde API y mezcla con default para compatibilidad.
    ...
    // Abre el editor contextual cargando los campos del bloque seleccionado.
    function openEditor(section) {
        ...
        // Helper para extraer párrafos
        const getPara = (obj) => {
            if (Array.isArray(obj.paragraphs_ES)) return obj.paragraphs_ES.join('\n\n');
            if (typeof obj.paragraph_ES === 'string') return obj.paragraph_ES;
            if (Array.isArray(obj.paragraphs)) return obj.paragraphs.join('\n\n');
            if (typeof obj.paragraph === 'string') return obj.paragraph;
            return '';
        };

        const getParaEN = (obj) => {
            if (Array.isArray(obj.paragraphs_EN)) return obj.paragraphs_EN.join('\n\n');
            if (typeof obj.paragraph_EN === 'string') return obj.paragraph_EN;
            return '';
        };

        _bodyES = getPara(sec) || getPara(dft) || '';
        _bodyEN = getParaEN(sec) || getParaEN(dft) || '';

        const legacyStatValue = sec.stat || dft.stat || '';
        const legacyStatLooksText = typeof legacyStatValue === 'string' && /[A-Za-zÁÉÍÓÚáéíóúÑñ]/.test(legacyStatValue);
        const fallbackStatES = legacyStatLooksText
            ? (sec.statLabel_ES || sec.statLabel || dft.statLabel_ES || dft.statLabel || legacyStatValue)
            : legacyStatValue;
        const fallbackStatEN = legacyStatLooksText
            ? (sec.statLabel_EN || dft.statLabel_EN || fallbackStatES)
            : legacyStatValue;

        setForm({
            title_ES: sec.title_ES || sec.title || dft.title_ES || dft.title || '',
            title_EN: sec.title_EN || dft.title_EN || '',
            subtitle_ES: sec.subtitle_ES || sec.subtitle || dft.subtitle_ES || dft.subtitle || '',
            subtitle_EN: sec.subtitle_EN || dft.subtitle_EN || '',
            badge_ES: sec.badge_ES || sec.badge || dft.badge_ES || dft.badge || '',
            badge_EN: sec.badge_EN || dft.badge_EN || '',
            body_ES: _bodyES,
            body_EN: _bodyEN,
            image: sec.imagen || dft.imagen || null,
            stats: sec.stats ? JSON.parse(JSON.stringify(sec.stats)) : (dft.stats ? JSON.parse(JSON.stringify(dft.stats)) : []),
            stat_ES: sec.stat_ES || dft.stat_ES || fallbackStatES,
            stat_EN: sec.stat_EN || dft.stat_EN || fallbackStatEN,
            statLabel_ES: sec.statLabel_ES || sec.statLabel || dft.statLabel_ES || dft.statLabel || '',
            statLabel_EN: sec.statLabel_EN || dft.statLabel_EN || '',
            tagline_ES: sec.tagline_ES || sec.tagline || dft.tagline_ES || dft.tagline || '',
            tagline_EN: sec.tagline_EN || dft.tagline_EN || ''
        });
    ...
    // Sube imagen de sección y actualiza preview del formulario activo.
    ...
    // Persiste únicamente la sección activa para evitar sobreescrituras globales.
    const saveChanges = async () => {
        ...
        if (activeEdit === 'caborcaHoy') {
            ...
            // Migración y compatibilidad: soportar payload legado con `value` y `label`.
            newSectionData.stats = form.stats.map((s) => {
                const legacyLabel = s.label || '';
                const legacyValue = s.value || '';
                return {
                    ...s,
                    label_ES: s.label_ES ?? legacyLabel,
                    label_EN: s.label_EN ?? s.label_ES ?? legacyLabel,
                    value_ES: s.value_ES ?? legacyValue,
                    value_EN: s.value_EN ?? s.value_ES ?? legacyValue
                };
            });
        } else {
        ...
        if (activeEdit === 'proceso') {
            // Compatibilidad con datos legacy que solo tienen `stat`
            newSectionData.stat_ES = form.stat_ES || previousState.stat_ES || previousState.stat || '';
            newSectionData.stat_EN = form.stat_EN || previousState.stat_EN || previousState.stat_ES || previousState.stat || '';
    ...
    // Guardado completo utilizado por el botón superior de publicar.
    ...
    const getProcesoStatValue = (obj) => {
        const localizedValue = currentProp(obj, 'stat');
        if (obj?.stat_ES || obj?.stat_EN) return localizedValue;

        const legacyValue = obj?.stat || '';
        const legacyLooksText = typeof legacyValue === 'string' && /[A-Za-zÁÉÍÓÚáéíóúÑñ]/.test(legacyValue);
        if (legacyLooksText) {
            return currentProp(obj, 'statLabel') || legacyValue;
        }
    ...
    const getStatValue = (stat) => {
        const localizedValue = currentProp(stat, 'value');
        if (stat?.value_ES || stat?.value_EN) return localizedValue;

        const legacyValue = stat?.value || '';
        const legacyLooksText = typeof legacyValue === 'string' && /[A-Za-zÁÉÍÓÚáéíóúÑñ]/.test(legacyValue);
        if (legacyLooksText) {
            return currentProp(stat, 'label') || legacyValue;
        }
                            ...
                            <div className="grid lg:grid-cols-2 gap-16 items-center">
                                <div>
                                    <div className="bg-white rounded-2xl p-8 shadow-2xl">
                                        <div className="w-full h-95 rounded-lg flex items-center justify-center">
                        ...
                        <div className="max-w-6xl mx-auto text-center">
                            ...
                            <div className="grid md:grid-cols-4 gap-8 mb-12">
                                {content.caborcaHoy.stats.map((s, i) => (
                                    <div key={i}>
                                        <div className="text-5xl md:text-6xl font-bold">{getStatValue(s)}</div>
                            ...
                            <div className="grid lg:grid-cols-2 gap-16 items-center">
                                ...
                                <div>
                                    <div className="bg-gray-50 rounded-2xl p-10 shadow-2xl">
                                        <div className="w-full h-105 flex items-center justify-center">
                            ...
                            <div className="grid lg:grid-cols-2 gap-16 items-center">
                                ...
                                <div>
                                    ...
                                    <div className="text-white rounded-lg px-8 py-6" style={{ backgroundColor: '#332B1E' }}>
                                        <div className="text-4xl md:text-5xl font-bold">{getProcesoStatValue(content.proceso)}</div>
                        ...
                        <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl p-6 m-4 absolute top-10">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                {['hero', 'origen', 'crecimiento', 'artesania', 'proceso'].includes(activeEdit) && (
                                    <label className="block">
                                        <div className="text-sm font-semibold text-gray-700 mb-1">Badge / Insignia</div>
                                        <input
                                ...
                                {activeEdit === 'legado' && (
                                    <label className="block">
                                        <div className="text-sm font-semibold text-gray-700 mb-1">Frase Final</div>
                                        <input
                                        ...
                                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                            {form.stats.map((stat, i) => (
                                                <div key={i} className="flex flex-col gap-1 bg-white p-2 rounded shadow-sm">
                                                    ...
                                                    <input
                                                        value={idioma === 'en'
                                                            ? (stat.value_EN || stat.value_ES || ((typeof stat.value === 'string' && /[A-Za-zÁÉÍÓÚáéíóúÑñ]/.test(stat.value)) ? (stat.label_EN || stat.label_ES || stat.value) : stat.value) || '')
                                                            : (stat.value_ES || ((typeof stat.value === 'string' && /[A-Za-zÁÉÍÓÚáéíóúÑñ]/.test(stat.value)) ? (stat.label_ES || stat.label || stat.value) : stat.value) || '')}
                                                        onChange={e => handleStatChange(i, idioma === 'en' ? 'value_EN' : 'value_ES', e.target.value)}
                                                        placeholder="Valor"
                                ...
                                {activeEdit !== 'hero' && (
                                    <label className="block md:col-span-2">
                                        <div className="text-sm font-semibold text-gray-700 mb-1">
                                            {activeEdit === 'caborcaHoy' ? 'Contenido' : 'Contenido - separar párrafos con línea en blanco'}
                                        </div>
                                        <textarea
                                ...
                                {activeEdit === 'proceso' && (
                                    <label className="block md:col-span-2">
                                        ...
                                        <div className="flex flex-col sm:flex-row gap-2">
                                            <input
                                                name={idioma === 'en' ? 'stat_EN' : 'stat_ES'}
                                                value={idioma === 'en'
                                                    ? (form.stat_EN || form.stat_ES || '')
                                                    : (form.stat_ES || '')}
                                                onChange={handleInput}
                                                placeholder="Ej. +200"
